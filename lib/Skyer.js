...
const AbstractRoute      = require('./route/AbstractRoute');

const AbstractRouteMiddleware = require('./RouteMiddleware');
const AbstractAppMiddleware   = require('./AppMiddleware');
...
class Skyer {
  constructor( options ) {
    ...
    this._listen_callback = () => {
      console.log('sky application listen on port %s', this._listen_port);
    };
  }
  ...
  _loadGlobal() {
    ...
    const globalFile    = path.join(bootstrapPath, 'global');
  ...
  _loadSysMiddleware( prefix, Middleware, file ) {
    ...
    if ( !name ) {
      console.log('invalid sys Middleware file: %s', file);
      return;
    }
  ...
  _loadAppMiddleware( prefix, Middleware, file ) {
    let name = this.utils.parseMiddleware(Middleware);
    if ( !name ) {
      console.log('invalid app Middleware file: %s', file);
      return;
    }
  ...
  _loadRouteMiddleware( prefix, Middleware, file ) {
    ...
    if ( !name ) {
      ...
      console.log('invalid route Middleware file: %s', file);
      return;
    }
  ...
  _loadController( prefix, Controller, file ) {
    ...
    if ( !name ) {
      ...
      console.log('invalid controller file: %s', file);
      return;
    }
  ...
  _loadService( prefix, Service, file ) {
    ...
    if ( !name ) {
      console.log('invalid service file: %s', file);
      return;
    }
  ...
  _loadRoute( prefix, Route, file ) {
    ...
    if ( !name ) {
      console.log('invalid route file: %s', file);
      return;
    }
  ...
  static get RouteMiddleware() {
    return AbstractRouteMiddleware;
  }

  static get AppMiddleware() {
    return AbstractAppMiddleware;
  }
...
const pkginfo = require('pkginfo')(module, 'name', 'version', 'author');
