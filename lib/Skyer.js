...
class Skyer {
  constructor( options ) {
    ...
    // map for name/class
    this._middlewares_name_class_map = {};
    // map for name/instance
    this._middlewares_name_instance_map = {};
  ...
  _loadMiddlewares() {
    ...
    function load( type ) {
      return function () {
        ...
        const args = [type].concat(argArr);

        this._loadMiddlewareByType.apply(this, args);
      }.bind(this);
    }

    this._loadByType(FILE_TYPE.SYS_MIDDLEWARE, load.call(this, MIDDLEWARE_TYPE.SYS));
    this._loadByType(FILE_TYPE.APP_MIDDLEWARE, load.call(this, MIDDLEWARE_TYPE.APP));
    this._loadByType(FILE_TYPE.ROUTE_MIDDLEWARE, load.call(this, MIDDLEWARE_TYPE.ROUTE));
  ...
  _loadMiddlewareByType( type, prefix, Middleware, file ) {
    let name = this.utils.parseMiddleware(Middleware);
    if( !name ) {
      debug('invalid Middleware file: %s', file);
      return;
    }

    switch ( type ) {
      case MIDDLEWARE_TYPE.SYS:
        this._sys_middlewares.push(Middleware);
        this._middlewares_name_class_map[name] = Middleware;
        break;
      case MIDDLEWARE_TYPE.APP:
        this._app_middlewares.push(Middleware);
        this._middlewares_name_class_map[name] = Middleware;
        break;
      case MIDDLEWARE_TYPE.ROUTE:
        ...
        break;
      default:
        debug('invalid middleware type, with file:%s', file);
        break;
    }
  }
  ...
  static get defaultConfig() {
    // todo: support cli env variable override this.
    // like: SKYER_VIEW_PATH='/data/www/skyer/views'
    return {
      config_path: path.join(Skyer.rootPath, 'config'),
      route_path: path.join(Skyer.rootPath, 'app/routes'),
      model_path: path.join(Skyer.rootPath, 'app/models'),
