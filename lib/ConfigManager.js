...
class ConfigManager extends AbstractElement {
  ...
  _init() {
    ...
    let envConfig = this.skyer.utils.requireSafe(path.join(envConfigPath, env));

    if ( hasCommonConf ) {
      envConfig = Object.assign(this.skyer.utils.requireSafe(path.join(envConfigPath, 'common')), envConfig);
    }
    ...
    if ( hasLocalConf && env === 'development' ) {
      config.localConfig = this.skyer.utils.requireSafe(path.join(envConfigPath, 'local')) || {};
