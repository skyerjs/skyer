...
class ComponentManager extends AbstractElement {
  ...
  _loadUserComponents() {
    this.logger.info('>>>>>>>> Load user components');
  ...
  _systemComponentsHotUpdate() {
    this.logger.info('>>>>>>>> hotUpdate all system components');
  ...
  _buildAllComponents() {
    this.logger.info('>>>>>>>> build all components');
  ...
  registerComponent( tname, cname, componentOrFactory ) {
    ...
    this.logger.info('Register component tname:%s cname:%s', tName, cName, { pid: process.pid });
  ...
  addComponent( tname, cname, componentOrFactory ) {
    ...
    if ( this.state !== MANAGER_STATES.INIT ) {
      ...
      this.logger.info('Add component tname:%s cname:%s', tName, cName, { pid: process.pid });
  ...
  removeComponent( name ) {
    this.logger.info('Remove component %s', name, { pid: process.pid });
  ...
  registerServiceDiscovery( serviceDiscovery ) {
    if ( serviceDiscovery instanceof ServiceDiscovery ) {
      this.logger.info('Register serviceDiscovery name:%s', serviceDiscovery.name);
  ...
  hotUpdate() {
    this.logger.info('HotUpdate all components', { pid: process.pid });
  ...
  shutdown() {
    ...
    this.logger.info('Shutdown all service discovery', { pid: process.pid });
    ...
    this.logger.info('Shutdown all components', { pid: process.pid });
    for ( let name in this.components ) {
