...
class Application {
  ...
  _registerMiddlewaresByDefault() {
    ...
    this._registerMiddlewares(allMiddlewares);
  }
  ...
  _registerMiddlewaresByConfig() {
    ...
    const middlewares = middlewareItems.map(item => {
      // find middleware from app/sys middlewares by name
      const inst = this.skyer._sys_app_middlewares_map[item.name];
      if ( inst ) {
        inst.order = item.order;
      }
  ...
  _registerMiddlewares( middlewares ) {
    ...
    middlewares.forEach(middleware => {
      ...
      if ( !middlewareFunc || typeof middlewareFunc !== 'function' ) {
        debug('app load middleware [%s] by not register, due to it return false/invalid function', middleware.name);
        return;
      }
  ...
  _listen() {
    ...
    this.app.listen(this.skyer._listen_port, this.skyer._listen_default_callback);
  }
