...
class Application {
  ...
  _loadSettings() {
    const bpAppPath   = path.join(this.skyer.options.bootstrap_path, 'app');
    const bpAppModule = require(bpAppPath);

    // todo: export function or object
  ...
  _registerRoute( prefix, key, route ) {
    ...
    console.log('routerPrefix:%s', routerPrefix);
    ...
    // console.log('>>>>>>>>>> routeParams length: %s', route.routeParamsList.length);
    ...
    route.routeParamsList.forEach(routeParams => {
      ...
      if ( !action && typeof action !== 'function' ) {
        console.error('Cannot find action:%s for controller:%s prefix:%s', actionName, controllerName, controllerPrefix);
        return;
      }
      ...
      middlewares.forEach(middlewareAction => {
        ...
        if ( !action ) {
          console.error('Cannot find action:%s for middleware:%s prefix:%s', actionName, middlewareName, prefix);
          return;
        }
      ...
      console.log('register route: %s %s', routeParams.method, routeParams.path);
  ...
  _registerMiddlewares() {
    ...
    allMiddlewares.forEach(middleware => {
      ...
      console.log('app use middleware with order: %s  [%s]', middleware.order, middleware.name);
